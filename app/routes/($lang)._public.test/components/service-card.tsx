  ...
  return createHost(ps.children, h => (
    <article className="flex flex-1 flex-col justify-between gap-7 rounded-xl bg-white p-7 py-5 shadow-md transition-colors ease-in-out hover:bg-brand-light">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 md:justify-between lg:flex-row items-center">
        ...
        <p
          ...
          {...h.getProps(ServiceCard.Description)}
        />
      </div>
      <div className="flex justify-end capitalize">{h.get(ServiceCard.Action)}</div>
    </article>
  ))
}
